...
DEFAULT_OUTPUT_DIR   = r"C:\Users\laura\OneDrive\Documenti\UNIVCA\TERZO ANNO\TIROCINIO\MP-IDB-The-Malaria-Parasite-Image-Database-for-Image-Processing-and-Analysis\Malariae\crops"     # dove salvare i risultati
...
LAB_RADIUS_MIN_FACTOR = 0.4   # sotto  0.4 * MEDIAN  → troppo piccolo
LAB_RADIUS_MAX_FACTOR = 1.6  # sopra  2.2 * MEDIAN  → blob fuso (caso 0006-T)
...
def _validate_lab_contour(contour, offset, parasite_area: float) -> tuple[bool, str]:
    """
    ...
    # 1. Dimensione minima
    if area <= parasite_area * 1.8:
        return False, f"area troppo piccola ({area:.0f} ≤ {parasite_area*1.8:.0f})"

    # 2. Compattezza — filtra blob fusionati e forme aberranti
    perim = cv2.arcLength(contour, True)
    compactness = (4 * np.pi * area / perim ** 2) if perim > 0 else 0
    if compactness < LAB_MIN_COMPACTNESS:
        return False, f"compattezza troppo bassa ({compactness:.3f} < {LAB_MIN_COMPACTNESS})"
...
def extract_rbc_crop(img: np.ndarray, gt_mask: np.ndarray) -> dict:
    ...
    if contour is not None:
        valid, reason = _validate_lab_contour(contour, offset, parasite_area)
        if valid:
