...
============================================
Estrae automaticamente i crop dei globuli rossi infetti (RBC)
...
DEFAULT_DATASET_DIR  = r"C:\Users\laura\OneDrive\Documenti\UNIVCA\TERZO ANNO\TIROCINIO\MP-IDB-The-Malaria-Parasite-Image-Database-for-Image-Processing-and-Analysis\Ovale"   # cartella con img/ e gt/
DEFAULT_OUTPUT_DIR = r"C:\Users\laura\OneDrive\Documenti\UNIVCA\TERZO ANNO\TIROCINIO\MP-IDB-The-Malaria-Parasite-Image-Database-for-Image-Processing-and-Analysis\Ovale\crops"    # dove salvare i risultati
...
# Fallback 
MEDIAN_RBC_RADIUS_PX = 146        # raggio mediano stimato del dataset
...
def extract_rbc_crop(img: np.ndarray, gt_mask: np.ndarray) -> dict:
    """
    Pipeline a cascata:
      1. LAB segmentation + fitEllipse        
      2. Circular Hough Transform             (fallback geometrico)
      3. Centroide GT + raggio mediano        (fallback ultimo)

    Returns dict con chiavi:
        crop, coords, radius, method,
        parasite_area, centroid, lab_binary
    """
    result = {
        "crop": None, "coords": None, "radius": None,
    ...
    if contour is not None and cv2.contourArea(contour) > parasite_area * 1.8:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Esempi:
  python rbc_extractor.py
  python rbc_extractor.py --dataset /dati/MP-IDB --output /risultati
  python rbc_extractor.py --limit 20          # test rapido su 20 campioni
  python rbc_extractor.py --no-diag           # salta diagnostiche (più veloce)
  python rbc_extractor.py --show              # mostra finestra per ogni campione
        """
    )
    ...
    for i, (img_path, gt_path) in enumerate(pairs):
        ...
        if crop is not None and crop.size > 0:
            ...
            crop_path = stage_dir / f"{sample}.png"

            cv2.imwrite(str(crop_path), crop)

        else:
            print(f"[WARN] Crop non salvato per {sample}")

        method = result["method"]
        radius = result.get("radius", "—")
        icon   = "✓" if not any(k in method for k in ("fail","no_","error")) else "✗"
        print(f"{icon}  {method:<36}  r={radius}px")
