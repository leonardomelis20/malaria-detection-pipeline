...
OUTPUT_DIR = Path(
    r"C:\Users\laura\OneDrive\Documenti\UNIVCA\TERZO ANNO\TIROCINIO\MP-IDB-The-Malaria-Parasite-Image-Database-for-Image-Processing-and-Analysis\results\features\reddino_base"
)
...
#MODELLO
def build_reddino_feature_extractor(device):
    model = timm.create_model(
        "hf_hub:Snarcy/RedDino-base",
        pretrained=True,
    )
...
def validate_h5(output_path):
    ...
    with h5py.File(output_path, "r") as h5f:
        ...
        if features.shape[1] != 768:
            raise ValueError(f"Dimensione feature errata: {features.shape[1]}, atteso 768")
...
def main():
    print("Device:", DEVICE)
    transform = get_reddino_transform()
    model = build_reddino_feature_extractor(DEVICE)
